...
def write_dreadnaut(atoms, bonds, dre_filename):
    """Écrit un fichier au format attendu par le programme C"""

    num_atoms = len(atoms)

    # --------- Construction listes d'adjacence ----------
    neighbors = [[] for _ in range(num_atoms)]
    for a1, a2, _ in bonds:
        neighbors[a1 - 1].append(a2 - 1)
        neighbors[a2 - 1].append(a1 - 1)

    # --------- Couleurs par symbole chimique ----------
    color_map = {}
    colors = []
    next_color = 0

    for atom in atoms:
        if atom not in color_map:
            color_map[atom] = next_color
            next_color += 1
        colors.append(color_map[atom])
    ...
    with open(dre_filename, "w") as f:
        ...
        for nbrs in neighbors:
            ...
            line = " ".join(str(n) for n in nbrs_sorted)
            f.write(line + "\n")
